...
def initialize_database(username, password):
    ...
    try:
        current_app.logger.debug("Acquiring initialization lock...")
        db.session.execute(db.text("SELECT pg_advisory_lock(123456789)"))
        ...
        current_app.logger.debug("Releasing initialization lock...")
